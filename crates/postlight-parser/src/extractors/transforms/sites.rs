        ...
        match name {
            ...
            "medium.com::iframe" => {
                ...
                let yt_re = regex::Regex::new(
                    r"https://i\.embed\.ly/.+url=https://i\.ytimg\.com/vi/(\w+)/",
                )
                .expect("medium yt re");
                let figure = first_ancestor_matching(doc, id, "figure");
                if let Some(caps) = yt_re.captures(&thumb) {
                    let youtube_id = &caps[1];
                    doc.set_attr(id, "src", &format!("https://www.youtube.com/embed/{youtube_id}"));
            ...
            "wired.jp::img[data-original]" => {
                    ...
                    (doc.attr_of(id, "src"), doc.attr_of(id, "data-original"))
                {
                    if let (Ok(base), Ok(resolved)) =
                        (url::Url::parse(&src), url::Url::parse(&src))
                    {
            ...
            "www.buzzfeed.com::div.longform_custom_header_media" => {
                ...
                let has_source = !doc.select_ids_in(id, ".longform_header_image_source").is_empty();
            ...
            "www.fool.com::.caption img" => {
                ...
                if let Some(parent) = doc.parent_of(id) {
                    replace_with_html(doc, parent, &format!("<figure><img src=\"{src}\"/></figure>"));
                }
                None
            }
            ...
            | "news.nationalgeographic.com::.parsys.content" => {
                ...
                let is_image_group = first_child
                    ...
                    .unwrap_or(false);
                if is_image_group {
                    let container = first_child
                        .and_then(|c| doc.select_ids_in(c, ".media--medium__container").into_iter().next());
                    let data_container = container
                        .and_then(|c| doc.children_ids(c).into_iter().next());
